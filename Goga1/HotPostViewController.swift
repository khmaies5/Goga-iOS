...
class HotPostViewController: UIViewController {
    ...
    func test(){
        Alamofire.request(Utils.baseUrl+"/posts/withRatings").responseJSON { response in
            
            switch response.result {
            case .success(let value):
                let jsonArray = JSON(value)
                print("JSON: \(jsonArray["Posts"])")
                ...
                for json in jsonArray["Posts"] {
                  ...
                  //  print(3,json.1["rating"])
                    ...
                    print(3,JSON(json.1["rating"]))
                    let jsonobj = JSON(json.1["rating"])
                    for rArray in jsonobj{
                    rating.id = jsonobj["id"].string
                    rating.like = jsonobj["like"].string
                    rating.dislike = jsonobj["dislike"].string
                    rating.postId = jsonobj["postId"].string
                    rating.userId = jsonobj["userId"].string
                    }
    ...
    func fetchPhotos2() {
        ...
        Alamofire.request( Utils.baseUrl+"/posts/withRatingsDesc")
            .responseJSON { response -> Void in
                ...
                case .success(let value):
                    ...
                    for json in jsonArray["Posts"] {
                        ...
                        post.rating.append(rating)
                        print(post.totalDislike,post.totalLike)
                        self.arrPost.append(post)
                  ...
                  // self.arrPost.sorted(by: { $0.id! < $1.id! })
                    self.photos = self.arrPost
...
extension HotPostViewController: KolodaViewDelegate {
...
extension HotPostViewController{
...
extension HotPostViewController: KolodaViewDataSource {
